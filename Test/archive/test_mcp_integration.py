#!/usr/bin/env python3
"""
Test MCP integration for Chinese PDF processing.
Ensures all tests use the MCP tool at http://127.0.0.1:8000/mcp/
"""
...
class MCPIntegrationTester:
    """Comprehensive MCP integration tester for Chinese PDF processing."""
    ...
    def test_mcp_service_availability(self):
        ...
        try:
            ...
            if response.status_code in [200, 400, 406]:  # 400 and 406 are acceptable for MCP
    ...
    def test_mcp_tools_availability(self):
        ...
        try:
            ...
            # Test with a simple request to check if tool exists
            headers = {
                'Accept': 'application/json, text/event-stream',
    ...
    def run_comprehensive_mcp_test(self):
        """Run comprehensive test of MCP integration for Chinese PDF processing."""
        print("🧪 MCP Integration Test Suite for Chinese PDF Processing")
        print("=" * 70)
        ...
        # Test 4: Process each Chinese PDF via MCP
        results = []
        
        for pdf_path in chinese_pdfs:
            print(f"\n{'='*70}")
            print(f"🔬 Testing PDF via MCP: {os.path.basename(pdf_path)}")
            print(f"{'='*70}")
    ...
    def _save_test_results(self, results):
        ...
        results_file = f"Results/mcp_integration_test_results_{timestamp}.json"
...
def main():
    """Main function to run the MCP integration test."""
    tester = MCPIntegrationTester()
    success = tester.run_comprehensive_mcp_test()
    
    if success:
        print("\n🎉 All MCP integration tests completed successfully!")
        print("✅ All tests used the MCP tool at http://127.0.0.1:8000/mcp/")
    else:
        print("\n❌ Some MCP integration tests failed. Check the output above for details.")
